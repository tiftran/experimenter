...
describe("The `DesignForm` component for Addons", () => {
  ...
  const setup = () => {
    ...
    jest
      ...
      .mockImplementation(() => Promise.resolve(fromJS(mockSuccessResponse)));
  ...
  const rejectedSetUp = () => {
    ...
    jest
      .spyOn(Api, "makeApiRequest")
      .mockReturnValueOnce(fromJS(mockSuccessResponse))
  ...
  it("renders addon forms", async () => {
    ...
    expect(getByLabelText("Multiple add-ons")).toBeInTheDocument();
    expect(
      getByLabelText("A single add-on for all branches"),
    ).toBeInTheDocument();
    expect(getAllByLabelText("Signed Add-On URL")).toHaveLength(1);

    let num_of_variants = successResponse.variants.length;
    expect(getAllByLabelText("Branch Size")).toHaveLength(num_of_variants);
    expect(getAllByLabelText("Name")).toHaveLength(num_of_variants);
    expect(getAllByLabelText("Description")).toHaveLength(num_of_variants);
  ...
  it("click on branched addon radio button", async () => {
    setup();
    const designForm = await render(<DesignForm />);
    const { getAllByLabelText, getByLabelText, queryByTestId } = designForm;
    await wait(() => {
      expect(queryByTestId("spinner")).not.toBeInTheDocument();
    });
    expect(Api.makeApiRequest).toHaveBeenCalled();
    ...
    expect(getAllByLabelText("Signed Add-On URL")).toHaveLength(2);
  });
  ...
  it("removes branch 1", async () => {
    ...
    expect(getAllByLabelText("Branch Size")).toHaveLength(1);
  ...
  it("Adds a new branch", async () => {
    setup();
    const { getByText, getAllByLabelText, queryByTestId } = await render(
      <DesignForm />,
    );
    ...
    // 3 Sets of Fields
    expect(getAllByLabelText("Branch Size")).toHaveLength(3);
    expect(getAllByLabelText("Name")).toHaveLength(3);
    expect(getAllByLabelText("Description")).toHaveLength(3);
  });
  ...
  it("Saves and Redirects", async () => {
    ...
    setup();
    const { getByText, queryByTestId } = await render(<DesignForm />);
    await wait(() => {
      expect(queryByTestId("spinner")).not.toBeInTheDocument();
    });
